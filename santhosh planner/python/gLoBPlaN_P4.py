		...
		if(round(alpha*180/np.pi) != 0):
			...
			if(time_step < time_rev):
				...
				if(math.tan(alpha) > 0):
					...
					if(ang2 > ang1):
						ang1 = ang1 + 2*np.pi
				cv2.ellipse(img,(self.inRo(centre[0]),self.inRo(centre[1])),(self.inRo(abs(radi)),self.inRo(abs(radi))),0,ang1*180/np.pi,ang2*180/np.pi,255,1)

			else:
				cv2.circle(img,(self.inRo(centre[0]),self.inRo(centre[1])),self.inRo(abs(radi)),255,1)

		else:
			cv2.line(img,(self.inRo(loc1[0]),self.inRo(loc1[1])),(self.inRo(loc2[0]),self.inRo(loc2[1])),255,1)
	...
	def main_class(self):                                   #main
		...
		while(que_open.leng() > 0):
			...
			for action in action_set:                               #running the loop for creating new children
				...
				val0 = self.obstacle_check(node_current.location,node_successor_loc,action[3],action[4])
		...
		return solution,self.img_Heu,img,dts_path
...
solution,img_Heu,img_main,path_pts = obj.main_class()
print('###############')
...
#REMEMBER : CHANGE THE HEURISTIC FUNCTION FOR THE 2nd A* ALGORITHM.
