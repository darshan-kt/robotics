...
class Astar_BiCycle:
	def __init__(self,stGo,orientation,img):
		...
		# self.costs_heuristic_obs = brushObj.main()
		# self.obs_heuristic_max = np.max(self.costs_heuristic_obs)
...
img_orig = cv2.imread('MaInmAP.png',0)
# img_orig = cv2.imread('MaInmAP_mINi.png',0)
# img_orig = cv2.imread('MaInmAP_mINi2.png',0)

kernel = np.ones((3,3), np.uint8)
img_dilate = cv2.dilate(img_orig.copy(), kernel, iterations=1)

inf_stgo = [[90,465],[470,460]]
init_orien = -np.pi/2

obj = Astar_BiCycle(inf_stgo,init_orien,img_dilate.copy())
solution,img_main,path_way_pts = obj.main_class()
path_way_pts.reverse()

print('###############')
imgC = img_orig.copy()
cv2.circle(imgC,(obj.inRo(path_way_pts[0][2][0]),obj.inRo(path_way_pts[0][2][1])),2,155,-1)


glob_path_pts = [inf_stgo[0]]
for pt in path_way_pts:
	_,action_pts = obj.disp_path(imgC,pt[0],pt[1],pt[3])
...
for pt in glob_path_pts:
	imgC[obj.inRo(pt[1])][obj.inRo(pt[0])] = 255
