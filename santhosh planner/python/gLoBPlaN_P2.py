...
# Global Planner : Actions are generated using the kinematics of the vehicle from -pi/6 -> +pi/6. 
# Collision Check is done by checking the location of node in the map. 
# The orientation is included in the nodes for checking in Open and Closed list.
...
#collision checking starts here
	...
	def main_class(self):                                   #main
		...
		start_node = tree(start,self.orientation,0,[np.nan,np.nan],0,self.distance(start,goal))
		...
		while(que_open.leng() > 0):
			...
			for action in action_set:                               #running the loop for creating new children
				...
				node_successor_f = node_successor_g + (self.distance(node_successor_loc,goal)/self.velocity)
