...
with open('/home/chandler/imp_ws/rtk_wp5.csv') as csv_file:
...
plt.scatter(x_wpt, y_wpt, color = 'blue')  
plt.show()
...
# waypoint_list=waypoint_list[1800:] #1800 IMPORTANT LINE FOR WAYPOINT SWITCH
...
def closest_index(localized_point):
  ...
  if val>0:
    ...
    if(closest_idx==0 and temp>0):
      closest_idx=temp
  return closest_idx,lane_waypoints[closest_idx:closest_idx+22]   #22
...
def main():
     ...
     ros_publisher = rospy.Publisher('/globalTrajectory', Path, queue_size=50)  
